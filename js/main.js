...
if (photoWrap && heptEl) {
  ...
  // Track rotation angle from animation
  function updateRotation() {
    const transform = window.getComputedStyle(heptEl).transform;
  ...
  photoWrap.addEventListener('mouseenter', () => {
    ...
    // Freeze at current rotation, then animate to 0deg
    heptEl.style.transform = `rotate(${currentRotation}deg)`;
    setTimeout(() => {
      heptEl.style.transform = 'scale(1.18) rotate(0deg)';
    }, 10);
  });
...
function buildSolarSystem() {
  ...
  // 3 orbit circles — WIDER to prevent clipping
  const orbits = [95, 135, 175];
  orbits.forEach(r => {
  ...
  // Badge config: [id, orbitIndex, speed] — FASTER speeds
  const badges = [
    { id: 'badge0', orbitIdx: 0, speed: 1.2, startAngle: 0 },      // IT Support: inner, fastest
    { id: 'badge1', orbitIdx: 1, speed: 0.85, startAngle: 120 },   // Music: middle, medium
    { id: 'badge2', orbitIdx: 2, speed: 0.6, startAngle: 240 },    // Photo: outer, slower
  ];
...
uvToggle?.addEventListener('click', () => {
  ...
  if (isUV) {
    ...
    console.log('%c Hover to reveal...', 'color:#6d28d9;font-family:monospace;font-size:10px;font-style:italic');
    console.log('%c', 'font-size:1px');
    console.log('%c "Seven is the number of completeness"', 'color:#4c1d95;font-family:monospace;font-size:9px');
  } else {
    console.clear();
    console.log('%c MAT. — Michael Austin Tang', 'color:#00e05a;font-family:monospace;font-size:13px;font-weight:bold;');
  }
