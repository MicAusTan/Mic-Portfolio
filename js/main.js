...
(function() {
  ...
  const ALL_PRIZES = [
    ...
    { id:'RezBG',      src:'assets/RezBG.png',           name:'Rez',      effect:'glitch'  },
    { id:'naznasio',   src:'assets/naznasio.png',        name:'Naz🎨',    effect:'rainbow' },
    { id:'reifanblind',src:'assets/reifanblind.png',     name:'Reifan',   effect:'normal'  },
    { id:'kevinchris', src:'assets/kevinchris.png',      name:'Kevin😊',  effect:'fadeClone'},
    { id:'rahmadshy',  src:'assets/rahmadshy.png',       name:'Rahmad😶', effect:'vanish'  },
    { id:'reifanhair', src:'assets/reifanhair.png',      name:'Reifan🌿', effect:'burst'   },
  ];
  ...
  function returnToInventory(obj) {
    const prizeId = obj.prize.id;
    // For clone/split effects — remove ALL copies when any is returned
    if (obj.prize.effect === 'clone' || obj.prize.effect === 'split') {
      const copies = physObjects.filter(o => o.prize.id === prizeId);
      copies.forEach(o => {
        o.el.remove();
        const i = physObjects.indexOf(o);
        if (i !== -1) physObjects.splice(i, 1);
      });
    } else {
      obj.el.remove();
  ...
  function physTick() {
    ...
    physObjects.forEach(obj => {
      ...
      // Wall collisions — use actual w/h not square size
      const objW = obj.size, objH = obj.sizeH || obj.size;
      let hitWall = false;
      if (obj.x < 0) { obj.x = 0; obj.vx = Math.abs(obj.vx) * 0.8; hitWall = true; }
      if (obj.x + objW > VW) { obj.x = VW - objW; obj.vx = -Math.abs(obj.vx) * 0.8; hitWall = true; }
      if (obj.y < 0) { obj.y = 0; obj.vy = Math.abs(obj.vy) * 0.8; hitWall = true; }
      if (obj.y + objH > VH) { obj.y = VH - objH; obj.vy = -Math.abs(obj.vy) * 0.85; obj.vx *= 0.92; hitWall = true; }
      ...
      if (obj.prize.effect === 'fast' && speed > 3) {
        spawnSpeedLine(obj);
      }
  ...
  function triggerWallEffect(obj, speed) {
    switch(obj.prize.effect) {
      ...
      case 'rainbow':
        ...
        break;
      case 'vanish':
        // Rahmad shy — becomes invisible for 1.5s on bounce
        if (!obj.vanishing) {
          obj.vanishing = true;
          obj.el.style.transition = 'opacity 0.15s';
        ...
        break;
      case 'burst':
        // Reifan hair — explodes into a ring of image-ghosts that fade
        triggerBurst(obj);
        break;
      case 'fadeClone':
        if (!obj.isFadeClone) triggerFadeClone(obj);
        break;
    }
  }

  function spawnTrailDot(obj) {
    // Ghost image stamp trail instead of green circles
    const stamp = document.createElement('div');
    const w = obj.size, h = obj.sizeH || obj.size;
    stamp.style.cssText = `position:fixed;width:${w}px;height:${h}px;
      left:${obj.x}px;top:${obj.y}px;pointer-events:none;z-index:9498;
      border-radius:8px;overflow:hidden;opacity:0.45;
  ...
  function spawnSpeedLine(obj) {
    // Speed lines behind Naufal — short white streaks in motion direction
    const len = 18 + Math.random() * 22;
    const angle = Math.atan2(obj.vy, obj.vx) * 180 / Math.PI + 180;
    const line = document.createElement('div');
    line.style.cssText = `position:fixed;
      width:${len}px;height:2px;
      left:${obj.x + obj.size/2}px;top:${obj.y + (obj.sizeH||obj.size)/2}px;
  ...
  function triggerFadeClone(obj) {
    // Spawn a clone that automatically fades out after 2.5s — no cap needed
    if (Date.now() - (obj.lastFadeClone||0) < 800) return; // 0.8s cooldown per obj
    obj.lastFadeClone = Date.now();
    spawnPhysObject(obj.prize, obj.x + obj.size/2 + (Math.random()-0.5)*80,
                    obj.y + (obj.sizeH||obj.size)/2, null);
    const clone = physObjects[physObjects.length-1];
    clone.isFadeClone = true;
    clone.vx = (Math.random()-0.5)*8; clone.vy = -Math.random()*5-2;
    // Auto-remove after 2.5s with fade
    clone.el.style.transition = 'opacity 0.6s';
    setTimeout(() => { clone.el.style.opacity = '0'; }, 1900);
    setTimeout(() => {
      clone.el.remove();
      const i = physObjects.indexOf(clone);
      if (i !== -1) physObjects.splice(i, 1);
    }, 2500);
  }

  function triggerGlitch(obj) {
    // RGB split + static flicker on the element
    if (obj.glitching) return;
    obj.glitching = true;
    let t = 0;
    const frames = 10;
    function glitchFrame() {
      ...
      obj.el.style.filter = `drop-shadow(${(Math.random()-0.5)*8}px 0 2px cyan) drop-shadow(${(Math.random()-0.5)*8}px 0 2px red) hue-rotate(${hue}deg)`;
      obj.el.style.transform = `translate(${ox}px,${oy}px) scaleX(${0.85 + Math.random()*0.3})`;
      requestAnimationFrame(glitchFrame);
    }
    requestAnimationFrame(glitchFrame);
  }

  function triggerRainbow(obj) {
    // Cycle hue on Naz face-painted — brief rainbow spin
    if (obj.rainbowing) return;
    obj.rainbowing = true;
    let h = 0;
    const id = setInterval(() => {
      h = (h + 25) % 360;
      obj.el.style.filter = `hue-rotate(${h}deg) saturate(1.8)`;
    }, 40);
    setTimeout(() => {
      clearInterval(id);
      obj.el.style.filter = '';
      obj.rainbowing = false;
    }, 1000);
  }

  function triggerBurst(obj) {
    // Reifan hair — spawns 6 fading image ghosts in a ring
    const cx = obj.x + obj.size/2;
    const cy = obj.y + (obj.sizeH||obj.size)/2;
    const ghostSize = Math.round(obj.size * 0.5);
    for (let i = 0; i < 6; i++) {
      ...
      ghost.style.cssText = `position:fixed;width:${ghostSize}px;height:${ghostSize}px;
        left:${cx - ghostSize/2}px;top:${cy - ghostSize/2}px;
