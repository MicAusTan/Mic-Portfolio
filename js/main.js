...
// ─── PAUSE HERO HEPT ON PHOTO HOVER + SNAP TO UPRIGHT + SCALE ───
...
if (photoWrap && heptEl) {
  photoWrap.addEventListener('mouseenter', () => {
    ...
    // Stop animation, lock at current angle
    heptEl.classList.add('paused');
    heptEl.style.transform = `rotate(${currentAngle}deg)`;
    
    // Force reflow to ensure the rotation is applied
    void heptEl.offsetWidth;
    
    // Add transition
    heptEl.style.transition = 'transform 1.1s cubic-bezier(.34,1.56,.64,1), opacity 1.1s ease';
    
    // After next frame, apply the target transform
    setTimeout(() => {
      heptEl.style.transform = 'scale(1.25) rotate(0deg)';
      heptEl.style.opacity = '0.65';
    }, 20);
  });

  photoWrap.addEventListener('mouseleave', () => {
    // Clear all inline styles
    heptEl.style.transition = 'transform 0.6s ease, opacity 0.6s ease';
