...
// ─── SCALE & SPEED UP RINGS ON PHOTO HOVER (NO ALIGNMENT) ───
...
if (photoWrap && heptEl) {
  photoWrap.addEventListener('mouseenter', () => {
    // Scale the entire heptagon container (all rings together)
    heptEl.style.transform = 'scale(1.15)';
    
    // Speed up each layer individually (keeps their rotation, just faster)
    const layers = heptEl.querySelectorAll('.hept-layer');
    layers.forEach(layer => {
      const currentDuration = parseFloat(window.getComputedStyle(layer).animationDuration);
      layer.dataset.originalDuration = currentDuration;
      // Speed up by 3x
      layer.style.animationDuration = (currentDuration / 3) + 's';
    });
  });
  ...
  photoWrap.addEventListener('mouseleave', () => {
    ...
    const layers = heptEl.querySelectorAll('.hept-layer');
    layers.forEach(layer => {
      if (layer.dataset.originalDuration) {
        layer.style.animationDuration = layer.dataset.originalDuration + 's';
      }
    });
  });
}
