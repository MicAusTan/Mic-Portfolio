...
function buildSolarSystem() {
  ...
  // Pulsing center glow ring behind the heptagon
  const pulseRing = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
  pulseRing.setAttribute('cx', cx);
  pulseRing.setAttribute('cy', cy);
  pulseRing.setAttribute('r', '52');
  pulseRing.setAttribute('fill', 'none');
  pulseRing.setAttribute('stroke', 'rgba(0,224,90,0.18)');
  pulseRing.setAttribute('stroke-width', '28');
  orbitSvg.appendChild(pulseRing);
  // Animate the pulse ring
  let pulseT = 0;
  function pulseCenterRing(now) {
    pulseT = now / 1800;
    const opacity = 0.06 + Math.abs(Math.sin(pulseT)) * 0.14;
    const radius  = 48 + Math.abs(Math.sin(pulseT * 0.7)) * 10;
    pulseRing.setAttribute('stroke', `rgba(0,224,90,${opacity.toFixed(3)})`);
    pulseRing.setAttribute('r', radius.toFixed(1));
    requestAnimationFrame(pulseCenterRing);
  }
  requestAnimationFrame(pulseCenterRing);
  ...
  // Draw heptagonal orbit paths — outer orbits slightly brighter
  orbits.forEach((r, i) => {
