...
// ─── PHOTO HOVER — pure JS, composited with parallax tilt ───
// CSS :hover scale is removed; we track hover state here so it composes cleanly
...
if (photoWrap && heptEl) {
  ...
  photoWrap.addEventListener('mouseenter', () => {
    ...
    photoWrap.style.transform = 'translate(-50%, -50%) scale(1.12)';
    const clip = document.getElementById('heroPhotoClip');
    if (clip) clip.style.filter = 'drop-shadow(0 0 32px rgba(0,224,90,0.6))';
  });
