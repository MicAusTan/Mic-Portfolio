...
// ─── PAUSE HERO HEPT ON PHOTO HOVER + SCALE + STRAIGHTEN ───
...
if (photoWrap && heptEl) {
  photoWrap.addEventListener('mouseenter', () => {
    // Find all ring layers
    const layers = heptEl.querySelectorAll('.hept-layer');
    
    layers.forEach(layer => {
      // Get current rotation of each layer
      ...
      // Pause animation and lock current rotation
      layer.style.animationPlayState = 'paused';
      layer.style.transform = `rotate(${currentAngle}deg)`;
      layer.style.transition = 'transform 1.3s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 1.3s ease';
    });
    
    // Animate all to upright + scaled
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        heptEl.style.transform = 'scale(1.28)';
        heptEl.style.opacity = '0.6';
        layers.forEach(layer => {
  ...
  photoWrap.addEventListener('mouseleave', () => {
    ...
    heptEl.style.transition = 'transform 0.8s ease, opacity 0.8s ease';
