...
(function() {
  ...
  function spawnPrizes() {
    prizes.length = 0;
    const pool = remaining.slice(0, 8);
    for (let i = 0; i < pool.length; i++) {
      prizes.push({
        prize: pool[i],
        x: MACHINE_LEFT + 30 + Math.random() * (MACHINE_RIGHT - MACHINE_LEFT - 80),
        y: MACHINE_TOP + 80 + Math.random() * (FLOOR - MACHINE_TOP - 140),
  ...
  dropBtn.addEventListener('click', () => {
    ...
    // Find what the claw might hit
    dropTargetY = FLOOR - 10;
    for (const p of prizes) {
      if (Math.abs(p.x + PRIZE_SIZE/2 - clawX) < PRIZE_SIZE * 0.6) {
        dropTargetY = Math.min(dropTargetY, p.y - 10);
      }
    }
  });
  ...
  function attemptGrab() {
    const candidates = prizes.filter(p =>
      Math.abs(p.x + PRIZE_SIZE/2 - clawX) < PRIZE_SIZE * 0.65 &&
      Math.abs(p.y + PRIZE_SIZE/2 - clawY) < PRIZE_SIZE * 0.65
    );
    if (!candidates.length) return null;
    if (Math.random() > 0.72) return null; // drop!
    return candidates[0];
  }
  ...
  function draw() {
    ...
    // Prizes
    prizes.forEach(p => {
      if (imgs[p.prize.id] && imgs[p.prize.id].complete) {
        ctx.save();
        ctx.beginPath();
        ctx.roundRect(p.x, p.y, PRIZE_SIZE, PRIZE_SIZE, 8);
        ctx.clip();
        ctx.drawImage(imgs[p.prize.id], p.x, p.y, PRIZE_SIZE, PRIZE_SIZE);
        ctx.restore();
        // Name tag
        ctx.fillStyle = 'rgba(0,0,0,0.65)';
        ctx.fillRect(p.x, p.y + PRIZE_SIZE - 14, PRIZE_SIZE, 14);
        ctx.fillStyle = '#6b9c7e';
        ctx.font = '8px Space Mono, monospace';
        ctx.textAlign = 'center';
        ctx.fillText(p.prize.name, p.x + PRIZE_SIZE/2, p.y + PRIZE_SIZE - 4);
      } else {
    ...
    if (grabbed && imgs[grabbed.prize.id]) {
      ctx.save();
      ctx.beginPath();
      ctx.roundRect(grabbed.x, grabbed.y, PRIZE_SIZE, PRIZE_SIZE, 8);
      ctx.clip();
      ctx.drawImage(imgs[grabbed.prize.id], grabbed.x, grabbed.y, PRIZE_SIZE, PRIZE_SIZE);
      ctx.restore();
    }
  ...
  function addToInventory(prize) {
    ...
    slot.innerHTML = `<img src="${prize.src}" alt="${prize.name}"><span class="inv-name">${prize.name}</span>`;
  ...
  function spawnPhysObject(prize, startX, startY, slotEl) {
    const SIZE = getPrizeDisplaySize(prize.effect);
    ...
    el.innerHTML = `<img src="${prize.src}" alt="${prize.name}">`;
  ...
  function getPrizeDisplaySize(effect) {
    return effect === 'grow' ? 80 : effect === 'split' ? 70 : 90;
  }
  ...
  function setupObjDrag(obj) {
    ...
    function onUp(e) {
      ...
      // Return to inventory if dropped over it
      if (isOverInventory(cx, cy)) {
  ...
  function physTick() {
    ...
    physObjects.forEach(obj => {
      ...
      // Grow effect
      if (obj.prize.effect === 'grow') {
        obj.growTime += 0.016;
        const newSize = Math.min(80 + obj.growTime * 12, 200);
        obj.size = newSize;
  ...
  function triggerWallEffect(obj, speed) {
    switch(obj.prize.effect) {
      case 'shake':
        ...
        break;
      case 'flash':
        flashScreen('rgba(255,255,220,0.55)', 120);
        break;
      case 'dark':
        flashScreen('rgba(0,0,0,0.75)', 200);
        break;
      case 'flip':
        ...
        break;
      case 'clone':
        if (speed > 7 && physObjects.filter(o=>o.prize.id===obj.prize.id).length < 4) {
          const clone = JSON.parse(JSON.stringify({prize:obj.prize}));
  ...
  function screenShake(intensity) {
    ...
    const dur = 18;
    function shake() {
      if (t++ > dur) { document.body.style.transform = ''; return; }
      const d = intensity * (1 - t/dur);
      document.body.style.transform = `translate(${(Math.random()-0.5)*d}px,${(Math.random()-0.5)*d}px)`;
  ...
  function flashScreen(color, ms) {
    ...
    fl.style.cssText = `position:fixed;inset:0;background:${color};z-index:9998;pointer-events:none;transition:opacity ${ms}ms`;
