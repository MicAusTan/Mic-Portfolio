...
// ─── BEFORE / AFTER SLIDER (experience panel) ───
(function() {
  const slider     = document.getElementById('expBaMini');
  const handle     = document.getElementById('expBaHandle');
  const beforeWrap = document.getElementById('expBaBeforeWrap');
  if (!slider || !handle || !beforeWrap) return;
...
(function() {
  ...
  const responses = {
    help:    ['Available commands:', '  status    — system status', '  ping      — test connection', '  whoami    — current user', '  tasks     — active responsibilities', '  skills    — loaded modules', '  clear     — clear terminal'],
    status:  ['[OK]  Hardware support: online', '[OK]  File management: active', '[OK]  User assistance: running', '[  ]  Formal IT certification: pending'],
    ping:    ['PING school-network.local', 'Reply from 192.168.1.1: time=2ms', 'Reply from 192.168.1.1: time=1ms', 'Packet loss: 0% — connection stable'],
    whoami:  ['michael-tang', 'Role: Student IT Support', 'School: Sekolah Harapan Utama', 'Status: Active since Jan 2024'],
    tasks:   ['1. Troubleshoot hardware issues      [daily]', '2. Software installation & setup     [weekly]', '3. Assist teachers with projectors   [daily]', '4. Organize shared digital files     [ongoing]'],
    skills:  ['[loaded]  troubleshooting v2.4', '[loaded]  file-management v1.8', '[loaded]  windows-os v10.0', '[loaded]  communication v3.1', '[loaded]  patience v9.9'],
    clear:   null,
  };
  ...
  input.addEventListener('keydown', e => {
    ...
    if (res) {
      res.forEach((line, i) => {
        setTimeout(() => addLine(line, i === 0 && res.length > 1 ? 'exp-term-response' : 'exp-term-response'), i * 60);
      });
    } else {
  ...
  // Click the card area to focus the input
