...
if (photoWrap && heptEl) {
  photoWrap.addEventListener('mouseenter', () => {
    ...
    // Pause animation and set current rotation
    heptEl.classList.add('paused');
    heptEl.style.transform = `rotate(${currentAngle}deg)`;
    heptEl.style.transition = 'transform 1s cubic-bezier(.34,1.56,.64,1), opacity 1s ease';
    
    // Animate to upright + scaled
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        heptEl.style.transform = 'scale(1.22) rotate(0deg)';
