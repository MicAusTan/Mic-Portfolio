...
(function() {
  ...
  handle.addEventListener('mousedown',  () => dragging = true);
  handle.addEventListener('touchstart', () => dragging = true, { passive: true });
  slider.addEventListener('mousedown',  e  => { if (e.target !== expandBtn && !expandBtn.contains(e.target)) { dragging = true; setPos(e.clientX); } });
  window.addEventListener('mousemove',  e  => { if (dragging) setPos(e.clientX); });
  window.addEventListener('touchmove',  e  => { if (dragging) setPos(e.touches[0].clientX); }, { passive: true });
  window.addEventListener('mouseup',    () => dragging = false);
  window.addEventListener('touchend',   () => dragging = false);

  prevBtn.addEventListener('click', () => loadExample(current - 1));
  nextBtn.addEventListener('click', () => loadExample(current + 1));

  // Expand to lightbox
  if (expandBtn) {
    expandBtn.addEventListener('click', e => {
      e.stopPropagation();
      openBaLightbox(current);
    });
  }
...
(function() {
  ...
  // BA drag for lightbox
  let lbDragging = false;
  function lbSetPos(clientX) {
    const rect = lbBaSlider.getBoundingClientRect();
    const pct  = Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
    lbBeforeWrap.style.width = (pct * 100) + '%';
    lbHandle.style.left    = (pct * 100) + '%';
  }
  if (lbHandle) {
    lbHandle.addEventListener('mousedown',   () => lbDragging = true);
    lbHandle.addEventListener('touchstart',  () => lbDragging = true, { passive: true });
    lbBaSlider && lbBaSlider.addEventListener('mousedown', e => { lbDragging = true; lbSetPos(e.clientX); });
  }
  ...
  window.openBaLightbox = function(idx) {
    ...
    lbAfter.src   = ex.after;
    lbBefore.src  = ex.before;
    lbBaCaption.textContent = ex.label;
    lbBeforeWrap.style.width = '50%';
    lbHandle.style.left    = '50%';
    open();
  };
